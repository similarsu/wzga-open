...
<jsp:include page="/includes/operate.info.jsp" />
 <div id="userAddBox" class="box1">
    <fieldset>
      <legend>新增用户</legend>
      <form:form name="userForm" id="userForm" action="${ctx}/sys/user/add" method="post" commandName="sysUser" onsubmit="return validateForm();" failAlert="表单填写不正确，请按要求填写！">
	  <table id="userAddTable" class="tableStyle" formMode="line" style="width:100%;">
		<tr>
		...
		<tr>
			...
			<td>
			  <form:password path="password" cssClass="validate[required,length[4,20]]" cssStyle="width:150px" />
			  <span class="star">*</span>
			</td>
		</tr>
		<tr>	
		...
		<tr>
			<td>身份证号：</td>
			<td>
			  <form:input path="identifyCode" cssClass="validate[required,custom[noSpecialCaracters]]" cssStyle="width:150px" onchange="ajaxIdcard()" />
			  <span class="star">*</span>
			</td>
		    <td>所属单位：</td>
			<td>
			  <input name="deptName" id="deptName" readonly="readonly" class="validate[required]" style="width:150px;" onclick="openHtml()" />
			  <span class="star">*</span> 
		...
		<tr>
			<td>隶属角色：</td>
			<td colspan="3">
			  <select prompt="请选择" data='<qjson:roleJSON type="select"/>' id="select" selWidth="150" />
...
<script type="text/javascript">
	function validateForm() {
		if (valFlag && valFlag2 && valFlag3) {
			...
			} else if ($("#identifyCode").val() != '' && !valFlag3) {
	...
	function ajaxLoginName() {
		if ($("#loginName").val() != '') {
			$.ajax({
				url : '${ctx}/ajax/user/loginName/isExists',
				type : 'POST',
				data : {
	...
	function ajaxPoliceCode() {
		if ($("#policeCode").val() != '') {
			$.ajax({
				url : '${ctx}/ajax/user/policeCode/isExists',
				type : 'POST',
				data : {
	...
	function ajaxIdcard() {
		if ($("#identifyCode").val() != '') {
			$.ajax({
				url : '${ctx}/ajax/user/identifyCode/isExists',
				type : 'POST',
				data : {
